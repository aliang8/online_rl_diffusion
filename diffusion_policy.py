...
class MLPBandit(nn.Module):
    """
    MLP Model
    """

    def __init__(self, action_dim, device, t_dim=16):
...
class Diffusion(nn.Module):
    ...
    def p_sample_loop(self, state, shape, verbose=False, return_diffusion=False):
        ...
        batch_size = shape[0]
        x = torch.randn(shape, device=device).float()
        ...
        for i in reversed(range(0, self.n_timesteps)):
            timesteps = torch.full((batch_size,), i, device=device).float()
            x = self.p_sample(x.float(), timesteps.long(), state.float())
    ...
    def p_losses(self, x_start, state, t, weights=1.0):
        ...
        # print(x_noisy.dtype, t.dtype, state.dtype)
        x_recon = self.model(x_noisy.float(), t.long(), state.float())
...
class DiffusionBandit(nn.Module):
    def __init__(
        self,
        action_dim,
        model,
    ...
    def p_sample_loop(self, shape, verbose=False, return_diffusion=False):
        ...
        for i in reversed(range(0, self.n_timesteps)):
            timesteps = torch.full((batch_size,), i, device=device).float()
            x = self.p_sample(x.float(), timesteps.long())
    ...
    def sample(self, batch_size, *args, **kwargs):
        shape = (batch_size, self.action_dim)
        action = self.p_sample_loop(shape, *args, **kwargs)
        # print(action)
        return action.clamp_(-self.max_action, self.max_action)
    ...
    def p_losses(self, x_start, t, weights=1.0):
        ...
        x_noisy = self.q_sample(x_start=x_start, t=t, noise=noise)
        x_recon = self.model(x_noisy.float(), t.long())
